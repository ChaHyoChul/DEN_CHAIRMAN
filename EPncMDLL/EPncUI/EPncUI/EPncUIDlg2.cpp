...
void CEPncUIDlg2::OnTimer(UINT_PTR nIDEvent)
{
	...
	else if( nIDEvent == 4 )
	{
		...
		updateRemainingMillingTimeDisp();	// ���� �ð� ���� 
		//
...
// 		nIndex++;
		...
		//////////////////////////////////////////////////////////////////////////
		// start ��ư:  origin * ready * nc file selected * runmode
		#ifndef _USE_PA_
...
// ���� �ð� ������ ȭ�鿡 ǥ�� 
// 2024.04.17 milling time�κп� Remain time�� ��� �ϰ�, Milling time�� finish time �κп� ��� �Ѵ� 
...
void CEPncUIDlg2::updateRemainingMillingTimeDisp()
{
	...
	CString strTemp;
	DWORD dwRemainTime = pa::PPAStatus->GetThreadState()->dwRunningTimeRemain;	// dwRunningTimeReamin�� sec ���� (line per sec) 
	CTimeSpan tms(dwRemainTime);

	int milling_time[2] = {tms.GetTotalMinutes(), tms.GetSeconds()};
	milling_time[1] = (int)((milling_time[1] / 10) * 10);	// 10�� ������ ��� �Ѵ� 
	strTemp.Format(_T("%02d:%02d"), milling_time[0], milling_time[1]);
...
void CEPncUIDlg2::updateProgressBarStatus()
{
	...
	case pa::NCFILE_STATE_RUNNING:
		// �������� ����Ѵ�, set rate based on total lines and current step
		...
		else 
		{
			...
			fCurrProgress = fCurrProgress > 99.0 ? 99.0 : fCurrProgress;	// �������� 100.0�� ���� �ʵ��� �Ѵ� //rather 99% until its finished 
			// Progress�� �����ϵ��� ���� ��Ų�� 
