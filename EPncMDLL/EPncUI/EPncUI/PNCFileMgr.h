...
public:
	...
	virtual void NCFMObsvr_UpdateNCFileList() = 0;
};

class INCFileMgrForSDObserver 
{
public:
	virtual void NCFMObsvrForSD_UpdateNCFileList() = 0;
};
...
// 	INCFileMgrObserver		*pObserver_;
	...
	std::vector<INCFileMgrForSDObserver*> vtrpSDObservers_;		// Observer 저장 
// 	INCFileMgrForSDObserver	*pSDObserver_;
...
public:
	...
	void RegisterSDObserver( INCFileMgrForSDObserver *pObserver );

	int GetNumNCFile();											// 등록된 NC 파일의 개수를 리턴 한다 
	int GetNumSelectedNcFile();									// 등록된 NC 파일 중 선택된 개수를 리턴한다 
...
// 	SNCFileInfo* GetCurrentNCFileInfo();						// 현재 작업중인 NC 파일 정보를 리턴 한다 
...
// 	void SetNCFileFinish( int nIndex, BOOL b );							// Finish 코드 
// 
// 	void SetNCFileState( int nIndex, pa::EN_NC_FILESTATE hState );	// State 코드 
// 
// 	void SetNCFileStartTime( int nIndex );								// 시작 시간 
// 
// 	void SetNCFileWorkTime( int nIndex, DWORD dwSec );					// 작업 시간 
// 
// 	void SetNCFileTotalLine( int nIndex, int nTotalLine );				// 파일의 총 라인 수 
// 
// 	void SetNCFileMachiningLine( int nIndex, int nMachiningLine );		// 가공 라인 번호 
	...
	void MoveUpWorkNCFileInfo( int nIndex, BOOL bNotify, BOOL bSort=FALSE );		// index 번째 작업 정보를 한단계 위로 이동 

	void MoveDownWorkNCFileInfo( int nIndex, BOOL bNotify, BOOL bSort=FALSE );		// index 번째 작업 정보를 한단계 아래로 이동 

		void MoveUpSelectedNcFile();	// 선택된 NC 파일을 모두 한단계 위로 이동 (사용안함)
		void MoveDownSelectedNcFile();	// 선택된 NC 파일을 모두 한단계 아래로 이동 (사용안함)
	...
	BOOL IsAutoAgingNCFile( CString strFileName );
	...
	void UpdateNcFileList();

	void UpdateNcFileListForSD();
