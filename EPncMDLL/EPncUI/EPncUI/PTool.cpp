...
void pa::CPTool::SetToolMaximumTime( int nToolNo, DWORD dwMaximumTime )
{
	...
	if( pToolData_->hTool[nToolNo].dwErrCode == ERR_TOOL_OVERTIME && (!(pa::PTool->GetEnableToolUsageTime()) || pToolData_->hTool[nToolNo].dwUsingTime < pToolData_->hTool[nToolNo].dwMaximumTime)){
		pToolData_->hTool[nToolNo].dwErrCode = 0;
	}
...
// 		// tool no에 해당하는 툴의 사용율을 확인한다
// 		if( pToolData_->hTool[toolNo].dwErrCode == 0 && pToolData_->hTool[toolNo].fUsingRate < 99.0 ) {
// 			break;
// 		}
// 		// 다음 연관툴을 확인한다 
// 		t += 2;
