...
//	void Melody( pa::EN_MELODY hMelody );
	...
	void ResetToolUsingTime( int nToolNo, BOOL resetCurrentTime, DWORD dwMaximumTime );

	void ToolErrorClear( int nToolNo );

	void ToolSetNumber( int nToolNo, int nTool2No );

	void SetSpindleRPM( int newRPM );
	...
	void UploadIPAddress();						// ip주소를 읽어서 저장한다 
	...
	void ResetOrigin( int axis_no );			// 원점 재설정 기능
