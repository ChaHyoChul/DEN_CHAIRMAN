...
// 	"WMINL",
	...
	"SORZ",
	"SMCT",			// PowerOn Test 명령 

	// set tool manually
	"WCTN",			// Change current tool number
...
BOOL pa::CPAAsyncComm::B_IS_RUNNING_COMPLETE[CMD_NUM];						// 동작중이면 TRUE, 그렇지 않으면 FALSE
DWORD pa::CPAAsyncComm::DW_SEND_RUNNING_COMMAND[CMD_NUM];					// 2017.05.22 
DWORD pa::CPAAsyncComm::DW_RECV_RUNNING_COMMAND[CMD_NUM];					//	명령을 보낸시간과 응답을 받은 시간을 저장하고, 에러 났을때 로그에 남긴다 
DWORD pa::CPAAsyncComm::DW_SEND_RECV_MAX[CMD_NUM];							// 
pa::EN_COORDINATE		pa::CPAAsyncComm::H_TEMP_COORDINATE_NO;				//
pa::EN_TEACHING_POINT	pa::CPAAsyncComm::H_TEMP_TEACHING_POINT_NO;			// 업로드한 Teaching Point 번호 
...
void pa::CPAAsyncComm::OnRecv( const char* p, int len )
{
	...
	char szTempResponse[2048];	// 응답 버퍼 
...
// 	}
	...
	{
		//////////////////////////////////////////////////////////////////////////
		// 명령에 포함된 데이터까지 다 받았는지 확인 
		// 응답 데이터는 [command param\r\n] 
...
void pa::CPAAsyncComm::process_response( char* pResponse )
{
	...
	// 응답 Command를 찾아서 command index를 저장 한다 
...
// 	}
	...
	if( response_command != -1 ) 
	{
		DW_RECV_RUNNING_COMMAND[response_command] = GetTickCount();		// 응답은 받은 것 이므로, 시간을 저장한다 
		// 응답 대기 최대 시간 저장 
		...
		if( pDataPtr != NULL ) 
		{
			...
			if( response_command == pa::CPAAsyncComm::CMD_RTCP ) {
				...
				sprintf_s( szResponse_, 512, "%s", pDataPtr );	// 응답 데이터를 저장한다 
			}

			//////////////////////////////////////////////////////////////////////////
			// 데이터에 에러 코드가 있는지 확인 
			// 에러가 있더라도, STATUS, STATE는 처리 해야 한다 
			...
			if( cErrCode == 'E' || cErrCode == 'e' )
			{
				...
				// 상태 전송 명령은 따로 처리 
				// 2016.06.23 응답에 NULL이 있을 경우 처리 기능 수정 
				if( response_command == CMD_RND_STATUS ) 
				{
...
// 					parsing_rnd_status_data( p );
				...
				{
// 					char *p = strstr( pDataPtr, "," ) + 1;	// RND_STATE E-1234,0,0,.... 에러 번호 다음 ,로 구분 
// 					parsing_rnd_state_data( p, TRUE );
				...
				if( nErrorCode != 0 ) 
				{
					...
					if( response_command == CMD_SMCT ) {
						...
						CPAAsyncComm::DW_RESPONSE_SMCT_CMD = 99999;		// 에러 리턴 
						return ;
					}
					else if( response_command == CMD_RISM ) {
						CPAAsyncComm::N_ERRORCODE_RISMCT_CMD = nErrorCode;
						CPAAsyncComm::DW_RESPONSE_RISMCT_CMD = 99999;	// 에러 리턴 
						return ;
					}
					else {
						if( response_command == CMD_FZS ) { 
		...
		case CMD_RND_STATUS: // gm_status 처리 
...
// 			N_TEMP_JOG_SPEED = ntemp[0];
		...
		case CMD_RZOO:	// read z-origin data 
			...
			break;
		case CMD_RTOO:	//RTOO -31.576,1 ; RTOO 31.576,2
			parsing_data( pDataPtr, ',', (double*)ftemp );
			if (ftemp[1] == 1)
			{
				PConfig->pConfig_->fOptionData[OPTION_Z1AXIS_ORIGIN_OFFSET] = ftemp[0];
				...
				// log 기록 
				{
					_stprintf_s( SZ_TEMP, 128, _T("[OPTION_Z1AXIS_ORIGIN_OFFSET] %.3f"), PConfig->pConfig_->fOptionData[OPTION_Z1AXIS_ORIGIN_OFFSET] );
					PThread->writeLog_UploadData( (LPCTSTR)SZ_TEMP );
				}
			}
			else if ( ftemp[1] == 2 )
			{
				PConfig->pConfig_->fOptionData[OPTION_Z2AXIS_ORIGIN_OFFSET] = ftemp[0];
				//////////////////////////////////////////////////////////////////////////
				// log 기록 
				{
					_stprintf_s( SZ_TEMP, 128, _T("[OPTION_Z2AXIS_ORIGIN_OFFSET] %.3f"), PConfig->pConfig_->fOptionData[OPTION_Z2AXIS_ORIGIN_OFFSET] );
					PThread->writeLog_UploadData( (LPCTSTR)SZ_TEMP );
				}
			}
			break;
		case CMD_RTDATA:
			...
			if (strcmp(pData, "spindleoffset") == 0)
			{
				...
				// log 기록 
				{
					_stprintf_s( SZ_TEMP, 128, _T("[OPTION_SPINDLE_OFFSET] %.3f"), PConfig->pConfig_->fOptionData[OPTION_SPINDLE_OFFSET] );
					PThread->writeLog_UploadData( (LPCTSTR)SZ_TEMP );
				}
			}
			else if (strcmp(pData, "spindle1runtime") == 0)
			{
				PPAStatus->GetThreadState()->dwTOTAL_LEFT_SPINDLE_RUN_TIME = ftemp[0];
				//////////////////////////////////////////////////////////////////////////
				// log 기록 
				{
					_stprintf_s( SZ_TEMP, 128, _T("[SPINDLE1_RUNTIME] %.3f"), PConfig->pConfig_->fOptionData[OPTION_SPINDLE_OFFSET] );
					PThread->writeLog_UploadData( (LPCTSTR)SZ_TEMP );
				}	
			}
			else if (strcmp(pData, "spindle2runtime") == 0)
			{
				PPAStatus->GetThreadState()->dwTOTAL_RIGHT_SPINDLE_RUN_TIME = ftemp[0];
				//////////////////////////////////////////////////////////////////////////
				// log 기록 
				{
					_stprintf_s( SZ_TEMP, 128, _T("[SPINDLE2_RUNTIME] %.3f"), PConfig->pConfig_->fOptionData[OPTION_SPINDLE_OFFSET] );
					PThread->writeLog_UploadData( (LPCTSTR)SZ_TEMP );
				}				
			}
			break;
		...
		case CMD_RND_GET_MEASURE_RESULT:
			// 데이터를 CThreadState::fTempMeasureResult 변수에 저장 한다 
			parsing_data( pDataPtr, ',', (double*)ftemp );
		...
		case CMD_GWVF:			// Read. WET/DRY
			parsing_data( pDataPtr, ',', (int*)ntemp );
			PConfig->pConfig_->nSelectM28Operation = ntemp[0];		// Wet/Dry 정보 저장 
			break;
...
//////////////////////////////////////////////////////////////////////////
// RND_STATE 명령의 응답을 parsing 한다 
//  0. io board connect 
//  1. spindle board connect 
//	2. tool no
//  3. too length	=> 2016.06.22 추가 
...
//	11. cantops input
//	12. cantops output
//  13. tool length update flag		=> 2017.08.22 추가 
//	14. rnd_errormsg 
//	x. rnd_errorcode : 없어짐 
	...
	for( int i = 0; i<len; i++ ) 
	{
		...
		if( ( pData[i] == ',' ) || ( i == len-1 ) )
		{
			if( szTemp[nTempIndex-1] == ',' ) {
				szTemp[nTempIndex-1] = 0;	// ','를 제거 하고, 문자열을 만든다 
			} else {
				szTemp[nTempIndex] = 0;		// ','가 없으므로, 문자열만 만든다 
			}

			switch( nResultIndex )
			{
			case 0: // I/O Board 상태 (1이면 정상, 0이면 에러)
				pRefPAStatus_->nIO_Board_Status = atoi( szTemp );
				break;
			case 1:	// Spindle Board 상태 (1이면 정상, 0이면 에러)
...
// 					TRACE( strDbg );
				...
				break;
			case 3:	// Tool Length. 중국 4WA는 이 기능이 없어서 주석처리 해야함. tool sensor까지 거리. offset 값을 더하면 coordinate 의 z 위치가 됨  
			...
			case 9: // system input
				...
				//	for( int i = 0; i<4; i++ ) {
				{
				//	int bit_no = (pa::PConfig->pConfig_->nModelID == 0) ? 4: 8;	// 5Z일 경우 4, 나머지는 8
					int bit_no = 8;
			...
			case 11: // captios input
				...
				//	for( int i = 0; i<16; i++ ) {
				{
				//	int bit_no	= (pa::PConfig->pConfig_->nModelID == 0) ? 16: 32;	// 5Z일 경우 4, 나머지는 8
...
// gm_status 명령에 대한 응답을 parsing 한다 
void pa::CPAAsyncComm::parsing_rnd_status_data( char* p )
{
	...
	for( int i = 0; i<len; i++ )
	{
		...
		if( p[i] == ',' || p[i] == NULL ) 
		{
			sztemp[nIndex-1] = ':';	// 데이터 마지막에 ':'를 붙여서 전달한다 
	...
	//////////////////////////////////////////////////////////////////////////
	// 데이터 파싱이 끝났으면, 속도 데이터를 사용해서 nMotorStopFlag 를 저장한다 
	int nMotorMoving = 0;
...
DWORD pa::CPAAsyncComm::parsing_io_data( char* p )
{
	...
	for( int i = 0; i<len; i++ )
	{
		...
		dwRet |= ( dwTemp << ( i * 4 ) );								// bit shift 해서 bit or 연산 한다 
	}
...
void pa::CPAAsyncComm::parsing_rnd_cdt_data( char* pData )
{
	...
	for( int i = 0; i<len; i++ ) 
	{
		...
		{
			if( szTemp[nTempIndex-1] == ',' ) {
				szTemp[nTempIndex-1] = 0;	// '123,'일 경우, '123' 으로 
			} 
			else {
				szTemp[nTempIndex] = 0;		// '123'일 경우, '123' 으로 
			}
...
// 			case 13:	// VC: 
			case 8:		// SV:0:0:0
				szTemp[nTempIndex-1] = ':';	// 데이터 마지막에 ':'를 붙여서 전달한다 
			...
			case 9:		// ToolNo
				pRefPAStatus_->nCurrentToolNo = atoi(szTemp);
				break;
			case 10:	// Tool Length. 중국 4WA는 이 기능이 없어서 주석처리 해야함. tool sensor까지 거리. offset 값을 더하면 coordinate 의 z 위치가 됨  
				pRefPAStatus_->fCurrentToolLenght = atof(szTemp);
				break;
			...
			case 19:	// bit-field data :  
				{
					...
					pRefPAStatus_->nMotorMovingFlag				= (nTemp & 0x00000040) == 0 ? 0 : 1;	// 모터이동중 1, 멈춤 0
				}
				break;

				// 2020.06.09. PA 제어기에서 올라오는 에러 메시지 저장 
