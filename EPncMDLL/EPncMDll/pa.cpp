...
BOOL pa::PA_INITIALIZE( char* pIpAddr, int nPortNo1, int nPortNo2, int nPortNoForStream, CString& strErrMsg )
{
...
#ifdef _DEBUG
	Sleep( 1*1000 );	// 제어기 부팅까지 1초간 대기 
#else
	Sleep( 1*1000 );	// 제어기 부팅까지 30초간 대기 
#endif 
	...
	// 데이터를 제어기에 저장할 경우, 제어기에서 읽어 온다 
	try {
#ifdef _USE_PA_
		// SMCT 명령 전송 (2017.05.01)
		//	- 2017.05.19 버전에는 적용하지 않는다 
		DWORD	dwErr = 0;
		DWORD	dwSysErr = 0;
		int		nRespErrCode = 0;
		int		nRespSysErrCode = 0;

		pa::PPAStatus->GetThreadState()->nIsConnectedIOBoard = 1;		// IO Board Connection flag 1로 초기화
		...
		// PAStatus::Initialize() 함수에서 읽었음 
		// - coordinate offset data range도 읽었음
		// - tool pocket range 파라메타도 읽었음 (poweron_test_config.ini)
        
		// coordinate offset 데이터 범위 에러 확인 
		coord_offset = CHECK_COORD_OFFSET_RANGE();
        
		// teaching point 데이터 범위 에러 확인 
		teaching_point = CHECK_TEACH_POINT_RANGE();
        
		// 응답에 에러가 있는지 확인 
		// 에러가 있다면, 에러 정보를 표시해 주고 계속 진행 여부를 확인 한다 
		if( ShowPowerOnTestFailDialog( dwErr, dwSysErr, nRespErrCode, nRespSysErrCode, coord_offset, teaching_point ) == IDCANCEL )
		{
			// Turn off power 메시지 출력 
			return FALSE;
		}
#endif 
	}
	...
	try
	{
		...
		// 2018.02.27 UsingAirLimitSensor, nAirLimitInterval 변수 데이터를 제어기로 전송한다 
		...
		// 2018.05.16 워터 플로우 센서 파리메타를 제어기로 전달 한다 
	...
	// 아래 기능을 수정 -> 파일을 모두 deselect 하고, 최근 사용한 파일도 열지 않는다 
	pa::PThread->CloseNCFile();
	pa::PNCFileMgr->DeselectAllNCFiles( FALSE );

	PPAStatus->GetThreadState()->bCompleteResetOrigin_  = TRUE;		// 초기 값은 true, 동작중일 경우 false
...
int pa::ShowPowerOnTestFailDialog( DWORD dwErrCode, DWORD dwSysErrCode, int nRespErrCode, int nRespSysErrCode, BOOL coord_offset, BOOL teach_point )
{
	...
	if( dwErrCode != 0 )
	{
		if( dwErrCode == 99999 ) 
		{
			// 명령 응답에 에러 E-nnn. nRespErrCode가 에러 코드
	...
	if( dwSysErrCode != 0 )
	{
		...
		{
			// 명령 응답이 E-nnn. nRespErrCode가 에러 코드
...
// Coordinate Offset 값의 범위를 비교 한다
...
BOOL pa:: CHECK_TEACH_POINT_RANGE()
{
	...
	// Tool Pocket 위치가 범위 안에 들어있는지 확인한다 
